...
router.post(
  ...
  async (req: PermissionRequest, res: Response) => {
    ...
    try {
      ...
      const result = await query(
        'INSERT INTO categories (user_id, name, type, color, icon, parent_id, organization_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
        [req.userId, name, type, color || null, icon || null, parentId || null, req.organizationId]
      );
...
router.post(
  ...
  async (req: PermissionRequest, res: Response) => {
    ...
    try {
      ...
      if (suggestion) {
        // Get category details and validate it belongs to user's organization
        const categoryResult = await query(
          'SELECT * FROM categories WHERE id = $1 AND organization_id = $2',
          [suggestion.categoryId, req.organizationId]
        );
